...
| [`probe_plugin_paths.py`](probe_plugin_paths.py) | Which of the paths in the document a reference server serves come from its plugins? | reference-target §1; [ADR-0007](../docs/decisions/0007-a-container-runtime-for-the-reference-instance.md) | no |
...
| [`probe_content_type_gate.py`](probe_content_type_gate.py) | When is a body's media type refused, and with what — is the condition a **missing** body or an **unreadable** one? | behaviours §1.11, §5; 007 §3.6, AC-25 | yes, and **only to an instance it creates and destroys** |
| [`probe_real_library_shapes.py`](probe_real_library_shapes.py) | Four shapes the fixture's silent music cannot answer: is `SeriesStudio` a derivation, do the nine by-name counts on a `MusicArtist` count anything, is a music container's `RunTimeTicks` a rollup, and what does `/Artists` actually list? | 005 §3.2, AC-26's tranche; behaviours §5, §5.3 | **no — every request is a `GET`** |
| [`probe_artist_registry.py`](probe_artist_registry.py) | What population does `/Artists` list, and what is one of its rows — is a row an item a client can follow, and is its identifier the one a track's own credits carry? | behaviours §5.3 | **no — every request is a `GET`** |
| [`probe_by_name_row_shape.py`](probe_by_name_row_shape.py) | Which properties a by-name row omits, and whether that is the route's rule, the type's, or the row's | 005 §3.2; behaviours §5.3 | **no — every request is a `GET`** |
| [`probe_latest_row_width.py`](probe_latest_row_width.py) | How wide is a `/Items/Latest` row — a list row, or a third width the way `/UserViews` is? | 005 §3.2, §3.7 | **no — every request is a `GET`** |
| [`probe_music_row_tranche.py`](probe_music_row_tranche.py) | What does a track's list row carry of 005 §3.2's audio tranche — `HasLyrics`, `PremiereDate`, `AlbumArtist`, `AlbumPrimaryImageTag` — and what decides each? | 005 §3.2, its owes list's causes 4 and 10 | **no — every request is a `GET`** |
| [`probe_playback_stream_fields.py`](probe_playback_stream_fields.py) | What does a media stream carry on a negotiation, and is each value a constant, a field of the file, or a composition? | 008 §3.1; 010's 2026-09-09 triage | **no — every request is a `GET`** |
| [`probe_playlists_folder.py`](probe_playlists_folder.py) | Where does the reference's playlists folder appear — `/UserViews`, a bare `GET /Items`, both, neither — and what hangs off it? | 009 §3.2; behaviours §5, §3.25; 005 §3.3 | yes, and **only to an instance it creates and destroys** |
...
| [`probe_first_time_setup.py`](probe_first_time_setup.py) | What does a Jellyfin answer while its first-time setup is unrun, and just after? The refusal envelopes, the edges, the library types it cannot scan, a setup that skips its configuration step, and whether a scan is waited for — and, in a second walk on the same instance, `/UserViews` over libraries of those types, the raw keys of a library row, the paths a library can be added with, and whether a refused rename leaves the password | 014 §7, OQ-4 to OQ-8; tasks T1, plan §6.4, §9 | yes, and **only to an instance it starts unconfigured and destroys** |
