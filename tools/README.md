...
| [`probe_subtitle_delivery.py`](probe_subtitle_delivery.py) | What do the subtitle playlist and the subtitle fetch answer — to a caller with no token, to a window, to a format, to a cue sitting exactly on a window boundary, and to every way of naming nothing? | 011 §3.5, §3.7, AC-10, OQ-6, OQ-8, OQ-11; behaviours §3.12 | only under `--allow-writes`: the image-subtitle case, which the reference attempts with ffmpeg before refusing |
