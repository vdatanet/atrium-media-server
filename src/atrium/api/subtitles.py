...
"""The reference's `SubtitleController`, fetch half: one track, one format, one window.

Two routes and one behaviour. `GET …/Subtitles/{index}/Stream.{format}` names the window in its
...
reference implements the second by calling the first, and so does this.

**Neither route requires a caller**, measured: no token, an unknown token and `?ApiKey=` answer
the identical `200` with the cues, where the subtitle *playlist* beside them refuses both of the
first two with an empty `401` `[probe: tools/probe_subtitle_delivery.py, Jellyfin 10.11.11,
...
## The four things this route does that no format specification predicts
...
## What it answers when it will not answer cues
...
Neither route answers `Accept-Ranges`, neither answers `Last-Modified` or an `ETag`, and a `Range`
header is ignored: the whole measured header set is `Content-Type` and `Content-Length`
`[probe: tools/probe_subtitle_delivery.py, Jellyfin 10.11.11, 2026-08-30]`.

See specs/011-subtitle-delivery/spec.md sections 3.5 and 3.7, and plan section 6.7.
"""
...
from atrium.media import extract, ffmpeg, subtitles
...
__all__ = ["WITHOUT_TICKS", "WITH_TICKS", "router"]
