...
#: **The count a Latest row carries when it stands for nothing but itself.** Every row of this
#: route carries `ChildCount`, and a row that was not grouped carries this
#: `[source: Jellyfin.Api/Controllers/UserLibraryController.cs:569-580 @ v10.11.11]`.
UNGROUPED_CHILD_COUNT = 0
    ...
    with session_scope(get_sessions(request)) as opened:
        ...
        while len(newest) < limit:
            ...
            for one in page.items:
                ...
                # **Grouping stops the moment there are `limit` groups, mid-page**, because the
                # group sizes are answered as `ChildCount` and a member counted after that point
                # is one the reference never saw: it walks its candidates in one pass and breaks
                # on `list.Count >= limit`
                # `[source: Emby.Server.Implementations/Library/UserViewManager.cs:GetLatestItems
                # @ v10.11.11]`. Counting to the end of the page instead made `Second Album`
                # answer 2 where the reference answers 1
        ...
        # **A Latest row is a list row plus `ChildCount`, and the property is not a field.** Every
        # row of this route carries it, whatever its type and whatever `fields` asked for - a film
        # carries `0` - and it is written over the built row rather than resolved through the
        # registry, which is where the reference writes it too
        # `[source: Jellyfin.Api/Controllers/UserLibraryController.cs:569-580 @ v10.11.11]`.
        #
        # **And it is not the container's child count.** It is the size of the group this row
        # stands for: on the reference's own fixture `Second Album` answers `1` here and `2` on
        # its full body, because one of its two tracks was among the latest
        # `[probe: tools/differential.py --fixture, Jellyfin 10.11.11, 2026-09-08]`. Reading it as
        # the container's own count is what the first attempt at this did, and it was wrong on
        # every grouped row and absent on every ungrouped one.
        for row, (key, _first) in zip(built, chosen, strict=True):
            row.child_count = members[key] if key in containers else UNGROUPED_CHILD_COUNT
