...
class BaseItemDto(AtriumModel):
    ...
    #: Always empty, and it is a statement about this server rather than about the item: no
    #: provider in v1 supplies a production location, so there is none to name. Measured present
    #: on **every** movie of a full body and on no other type, and on no list row of any type
    #: `[probe: tools/probe_wide_body_constants.py, Jellyfin 10.11.11, 2026-09-06]`.
    ...
    #: Always empty, for `ProductionLocations`' reason: nothing in v1 resolves a broadcast day.
    #: The one field of this tranche that travels on a **list row** as well as a full body -
    #: measured on all three series of the fixture, both widths, and on no other type.
    air_days: list[str] | None = None
    ...
    #: provider: trickplay tiles are images generated by scrubbing a video, and this server makes
    #: none. A map of maps on the wire; measured `{}` on every movie and episode of a full body
    #: and absent from every list row.
    trickplay: dict[str, dict[str, Any]] | None = None
