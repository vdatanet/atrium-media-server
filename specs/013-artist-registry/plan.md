...
| `ChildCount` answers `0` where the reference sends a stable number nobody has attributed | Certain | A differential difference on every registry row | Decision 2, declared in behaviours as a divergence with its argument, not excused in the allowlist as a derivation. **The attribution is worth one more reading before the row is written**: three registry artists against their tracks, their albums, and the albums their tracks sit in |
