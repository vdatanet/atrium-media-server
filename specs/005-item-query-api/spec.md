...
**And the wide widths gate by type as well, which a list row's matrix already did and this table
did not say.** Three more properties travel on a full body, on some types only, and on no list row
of any type. Measured over this repository's own fixture, every item of every type read as a full
body `[probe: tools/probe_wide_body_constants.py, Jellyfin 10.11.11, 2026-09-06]` — 32 of 32
movies, 9 of 9 episodes, 3 of 3 series, and none of the eight types on a list row:

| Field | Types | What it says |
|---|---|---|
| `ProductionLocations` | `Movie` | Always empty: no provider in v1 supplies a production location, so there is none to name |
| `Trickplay` | `Movie`, `Episode` | Always the empty map: trickplay tiles are images generated by scrubbing a video, and this server generates none |
| `AirDays` | `Series` — **and its list row too**, which is why it is in the matrix above rather than here | Always empty: nothing in v1 resolves a broadcast day |
| `SeriesStudio` | `Season`, `Episode` | The **parent series'** first studio, and the empty string where it has none — a derivation, not a constant |
| `AlbumCount`, `SongCount` | `MusicArtist` | The albums under the artist and the files under it. Real counts, from the same subtree rollup `ChildCount` uses |
| `TrailerCount`, `MovieCount`, `SeriesCount`, `ProgramCount`, `EpisodeCount`, `ArtistCount`, `MusicVideoCount` | `MusicArtist` | Always `0`: v1 models none of these anywhere under an artist |

> **Three of these were left open by [#308's tranche](tasks.md) and answered on 2026-09-06, on a
> library with real music** `[probe: tools/probe_real_library_shapes.py, Jellyfin 10.11.11,
> 2026-09-06]` — because each was empty on this repository's fixture, and an empty value can be a
> constant or a real value the fixture cannot produce.
>
> `SeriesStudio` is `""` on all 9 episodes and all 7 seasons here and one of the **series' own**
> studios on five sampled episodes of five there. The nine by-name counts are **two counts and
> seven zeros**: `AlbumCount` and `SongCount` answered 5 and 42, 3 and 55, 1 and 19 on the sampled
> artists while the other seven answered `0` on every one — so the seven join `ProductionLocations`
> as things this server can say truthfully, and the two are computed. And a music container's
> `CumulativeRunTimeTicks` is the **exact sum of its tracks**, so the `0` this fixture shows is the
> rollup of nothing rather than a constant: it is emitted as `0` on an album and an artist where
> every other type omits it.

> **Why these three and not the rest of the tranche.** All three are things this server can say
> truthfully as constants, because it resolves none of them and never will in v1 — which is
> exactly what separates them from `HasLyrics`, whose `false` would deny lyrics sitting in an
> `.lrc` beside the track, and from `SeriesStudio`, which looks like the constant `""` on a
> fixture with no studios and is a **derivation from the parent series** on a library that has
> them — this server resolves studios (`Studios` travels today). The rest of the tranche is on
> [005's list](tasks.md#what-this-feature-owes-the-next-ones) with what the measurement found.
...
12. `Similar` and `InstantMix` return identical results for identical input on repeated calls,
    ...
    [behaviours §3.24](../../docs/compatibility/behaviours.md).)*
13. `/Artists` and `/Artists/AlbumArtists` answer **two populations, and neither contains the
    other**: the distinct artists a **performer** credit names, and the distinct artists an
    album-artist credit names. A performer who is nobody's album artist is a row in the first and
    not the second; an album artist no track performs is a row in the second and not the first.
    The credit distinction is observable at item level too: `artistIds` finds the guest track and
    `albumArtistIds` does not.
    *(Restated by T14 — the draft asked the two routes to differ on the compilation, and under
    §5.3 there was no row that could show the difference. **Restated again on 2026-09-07 by
    [013](../013-artist-registry/spec.md) T8**, which built that row: as restated at T14 this
    criterion said the album-credit set is a **subset** of the any-credit set and that the two
    **coincide** for §5.3's reason, and both clauses are false once every credited name has a row.
    Its own test carried the tripwire — `assert album == every, "if these ever differ, behaviours
    5.3's argument needs rereading"` — and it fired. Neither containing the other is the measured
    shape `[probe: tools/probe_artist_registry.py, Jellyfin 10.11.11, 2026-09-07]`, where 11 of
    the reference's 506 album artists are named as a performer by no track at all.)*
...
26. The wide widths gate by type too (§3.2's 2026-09-06 amendment): `ProductionLocations` is on
    **every** full body of a `Movie`, `Trickplay` on every full body of a `Movie` and an
...
29. The wide widths carry three more per-type shapes, and all three are **derivations that look
    like constants on a fixture with no music** (§3.2): `SeriesStudio` on a `Season` and an
    `Episode` is the parent series' first studio and the empty string where it has none;
    `AlbumCount` and `SongCount` on a `MusicArtist` are the albums and the files under it, while
    its seven other by-name counts are `0`; and a music container's `CumulativeRunTimeTicks` is
