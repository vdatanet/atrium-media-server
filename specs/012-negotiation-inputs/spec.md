...
amended: 2026-08-30 by **011 T9** — unusually, an amendment made from another feature's task, and recorded as such because the measurement was taken there and the answer belongs here. OQ-4 answered the case-insensitive binding for the **protocol** value alone; measuring the same question for a delivery method's vocabulary showed the leniency is the *binder's* and not that enumeration's, because a direct-play entry typed `video` rather than `Video` binds and is answered a direct play. OQ-4 is therefore widened rather than corrected — the original answer was narrow, not wrong — and now names the four further enumerated values this body carries, each of them refused here with `400` where the reference answers `200`. Nothing else in this document moves, and no code changes with it: 011 T9 fixed only the vocabulary 011 added, and the general fix stays 012's to make; and 2026-09-03 at the **plan gate** — AC-10's second clause is withdrawn, and what made it wrong landed nine days after this document was accepted. It said a listed source's capability flags *"stay `true`: they are not a negotiation and nothing decides them"*; the negotiation's policy gate was then measured to be two rules rather than one, and its profile-less half — one permission per media kind — is what a **listing** answers as well, on both servers, because the reference builds an item body's media sources and a profile-less negotiation's from one function `[probe: tools/probe_playback_info.py, Jellyfin 10.11.11, 2026-09-02]`, `[source: Emby.Server.Implementations/Library/MediaSourceManager.cs:355-372 @ v10.11.11]`. The criterion's prohibition — *nothing this feature does changes what a listing answers* — is untouched and is the whole of what it was written for; the clause naming the values goes to 008, which states them. Nothing else in this document moves, and no code changes with it: 008's own fix had already implemented the rule. The three `true`s §3.1 and §3.2 record are an **administrator's**, this gate having measured from one seat; and 2026-09-03 at the **task gate** — §3.2's two ways to reach an un-inspectable source are two on the reference and **one here**. A zero-length file is skipped by 003's walk before it can become an item (`library/walker.py`'s `Skip.EMPTY`), measured in this repository that day against a real scan of one, while the reference admits it and answers both a listing and a negotiation for it `[probe: tools/probe_uninspected_source.py, Jellyfin 10.11.11, 2026-08-29]`. §3.2 and §6 say so now, the fixture builds the state the one way it can, and the difference itself is **003's** and is handed on rather than absorbed; and 2026-09-03 by **T1** — §3.3's refusal row gains the **message** beside the key it already named. Measured rather than read: the sentence names the enumeration by its fully qualified name, repeats the property's JSON path in `Path:` and carries a byte position, where the same failure on `POST /Playlists` is keyed `$`, says `Path: $` and counts the position inside the token instead. One failure, two shapes, two routes `[probe: tools/probe_playback_info.py, Jellyfin 10.11.11, 2026-09-03]`; and 2026-09-04 by **T3** — §3.2's statement of the reference's condition gains the clause it did not have. The refresh is declined outright for a source that stands in for something with no file of its own — a recording in progress, or a source with no path — so the condition is a conjunction and not the three-way choice this document described `[source: MediaBrowser.Controller/Entities/BaseItem.cs:1103, 1159 @ v10.11.11]`. Neither shape exists in v1, so nothing observable moves and no criterion changes: it is written down because a condition stated in part is one a later reader completes by guessing; and 2026-09-04 by **T4** — §4 gains a third row. The write D-1 decided on at the plan gate moves the opened file's **change signal** as well as its inspection, and §4 named only the second, so the observable a reader would have taken this document to promise was half of what the feature writes: a source healed in its streams, runtime, bitrate and size while keeping the entity tag of bytes nobody can play describes two files in one answer. The reference moves both across one heal `[probe: tools/probe_uninspected_source.py, Jellyfin 10.11.11, 2026-09-03]`, so this is parity and no criterion changes; and 2026-09-04 by **T5** — §3.2 gains the part this document never described. An item here can carry a source with no inspection **beside** one that has it, which is 003's two-part shape and a shape the reference has not got, and this document had only ever described the item all of whose sources are un-opened. That second part is answered by the same rule as the first — flags the ladder decided and an address beside them — rather than keeping the three advertised capabilities with nothing to fetch, because the rule §2.2 states is about the answer being actionable and not about which source made the server open a file. Nothing on the reference moves, there being no such source there, and no criterion changes: AC-1 and AC-4 are written over *a source nothing has opened* and this is one; and 2026-09-04 by **T7** — §3.3's first finding was measured on one vocabulary and stated of five, and measuring the other four found a fifth class nobody had asked about. Each of the four takes an altered spelling and each takes a **number**, and the number is the one the reference's own vocabulary declares rather than the position a reader would count: two of the four say so on the wire, one whose members are declared in a different order and one whose numbering skips a value, so a server counting positions answers a correct request with the wrong member and constrains the stream the client did not name. The fifth class is a number **no** member has, and it is three answers rather than one — two vocabularies ignore the entry carrying it, a third treats the condition as satisfied, and the fourth refuses the whole request with `500`. This server answers `400` to all four, which is the shape [behaviours §3.0.2](../../docs/compatibility/behaviours.md#302-what-is-never-acceptable) forbids — recorded at [behaviours §3.26](../../docs/compatibility/behaviours.md) with its two candidates and **left to its owner**, because it predates this feature and nothing here moves it. No criterion changes: AC-7 and AC-8 are written over the delivery protocol, whose four classes are unchanged, and the widening is a measurement of what the same rule does elsewhere `[probe: tools/probe_playback_info.py, Jellyfin 10.11.11, 2026-09-04]`; and 2026-09-04 by **T11** — AC-9 is amended, and what made it wrong is two of its own neighbours rather than anything measured. As accepted it read *"nothing in this feature changes what a negotiation answers for an item that **has** been opened, for any profile"*, and AC-7 and AC-8 change exactly that on purpose: a profile spelling the delivery protocol `Hls`, or `2`, or `dash` is answered differently after this feature than before it, on an item the scan opened. So no test could have passed both, and the closing task found the criterion by looking for the one that could fail rather than the one that mentions the behaviour — which is [010 T15's AC-2](../010-conformance-harness/tasks.md) again. The missing word is **which of the negotiation's two inputs** the prohibition is about: it is the file side, the resolution this feature adds, and the criterion says so now. Nothing observable moves, no code changes with it, and no test is weakened — the answers AC-7 and AC-8 require are asserted by their own rows of the acceptance map
depends_on: [003, 008]
---
...
**One state is exempt there and exists nowhere here**, read at T3 and written down so the condition
is stated whole: the reference does not re-read a source that stands in for something with no file
of its own — the source of a recording in progress, or a source with no path at all
`[source: MediaBrowser.Controller/Entities/BaseItem.cs:1103, 1159 @ v10.11.11]`. Neither shape is
in v1, which has no live television and gives every source the path of a file a scan found.
...
**A source with no inspection is not always the *only* source of its item, and on this server it
can sit beside one that has an inspection.** That shape is 003's — a two-part film whose second
part no prober will accept is one item with two sources here, where the reference keeps the
unreadable part as neither a source of the grouped item nor an item of its own
`[probe: tools/probe_uninspected_source.py, Jellyfin 10.11.11, 2026-09-03]` — so there is nothing
on the reference to be faithful to, and what decides it is §2.2's rule rather than a measurement.
**The part is answered exactly as the first paragraph's source is**: its capability flags are
decided against the profile and the capability it advertises carries an address. What does *not*
happen is the file being opened: the condition above is a property of the item, read off source
zero, and an item whose source zero is annotated is one this feature never probes (§3.1's row one
is about the item, not about each part). So the two halves come apart on this shape — nothing is
opened, and the answer is still actionable — and that is the rule working rather than an exception
to it.
...
container the extension claims — **and only the second of those two is reachable on this server**,
because a file of no length is skipped by the walk before it can become an item at all
(`Skip.EMPTY`, 003), where the reference admits one and answers a listing and a negotiation for it.
That is a difference of 003's rather than of this feature, found at this feature's task gate and
recorded there. §3.4's rows are written from what was measured rather than from
...
| `dash`, `" "`, `true` | `400`, RFC 9457 problem details, `errors` keyed on `$.DeviceProfile.TranscodingProfiles[0].Protocol`, whose one message names the enumeration, repeats that path and gives a byte position: *"The JSON value could not be converted to Jellyfin.Data.Enums.MediaStreamProtocol. Path: $.DeviceProfile.TranscodingProfiles[0].Protocol \| LineNumber: 0 \| BytePositionInLine: 398."* `[probe: tools/probe_playback_info.py, Jellyfin 10.11.11, 2026-09-03]` |
...
| The **change signal** of the file that negotiation opened | The healed source's entity tag on that listing: the opened file's own, rather than the one recorded when the file could not be read | The same lifetime, and the same row a rescan writes |
...
**The third row is the same answer applied to the same file's other half, and it is parity too.**
A source healed in its streams, its runtime, its bitrate and its size while keeping the entity tag
of bytes nobody can play describes two different files in one answer. The reference's on-demand
path moves both — measured across one heal, the tag and the size changing together
`[probe: tools/probe_uninspected_source.py, Jellyfin 10.11.11, 2026-09-03]` — so writing it here
reproduces rather than improves. It is still not new state: it is the row a rescan writes, written
from the request that read the file.
...
8. A profile whose protocol names no member answers by class, not by one rule: `400` with problem
   ...
   `1`.
9. Nothing this feature does to the **file side** of the negotiation's input changes what it
   answers for an item that **has** been opened, for any profile: the ladder, the reasons, the
   addresses and the flags of
   [008 §3.3](../008-playback-negotiation-and-delivery/spec.md#33-the-decision) are unchanged, and
   the existing conformance suite is the proof. The one thing that does move for an opened item is
   the **client side** — which member a profile's delivery protocol binds to, and the refusal for
   a value that binds to none — and that is AC-7 and AC-8, not a breach of this one. *(Amended on
   2026-09-04 by T11: as accepted this criterion read "nothing in this feature changes what a
   negotiation answers for an item that has been opened, for any profile", which is a prohibition
   its own two neighbours break deliberately — a profile spelling the protocol `Hls` is answered a
   different address after this feature than before it, on an item the scan opened. No test could
   have passed both, and the missing word was which of the negotiation's two inputs the
...
*fails* on: bytes that are not the container their extension claims. A file truncated to its first
kibibyte is **not** one of them — it probes cleanly — and neither, on **this** server, is a
zero-length one: it never becomes an item, so the conformance world builds the state the one way it
can (§3.2). `tools/probe_uninspected_source.py`
