...
|---|---|---|
| `GET /Users` | **L3** | The suite asks it and asserts the order, the filters and the document; the differential compares it from both seats |
| `POST /Users/New` | **L3** | The suite asserts the created document and every refusal of AC-3 and AC-4; the harness creates its seat with it |
| `POST /Users/{userId}/Policy` | **L3** | The suite asserts the round trip and the reachable refusals; the harness restricts its seat with it |
| `POST /Users/Password` | L2 | The suite asserts the three paths of §3.4. On no comparison's path, and it says so rather than claiming a level nothing pays for |
| `DELETE /Users/{userId}` | **L3** | The suite asserts AC-12; the harness destroys its seat with it |
...
**Four of the five are `L3`, decided at the plan gate on 2026-09-16 (OQ-2).** The harness needs no
new ability to reach them — it holds an administrator's token already — and AC-14 makes it *create*
the seat it compares, restrict it and destroy it. So `POST /Users/New`, `GET /Users`,
`POST /Users/{userId}/Policy` and `DELETE /Users/{userId}` are asked of both servers by a run that
has to happen anyway, which is what `L3` means. **`POST /Users/Password` is `L2`**: no comparison's
path reaches it, and a level nothing pays for is the claim 010's own gate exists to prevent.

Levels are defined in [../../docs/compatibility/conformance.md](../../docs/compatibility/conformance.md).

## 7. Open questions

**All seventeen are closed.** Fifteen on the day the feature opened — seven decided by the
operator, eight measured at its gate — and the last two at the plan gate the same day: OQ-2 (`L3`
for four of the five routes) and OQ-13 (a policy command reads before it writes, because the gate
made that compulsory).
...
| OQ-2 | ~~Are these five rows `L3`, or `L2` with `L3` owed to 014's own L3 change?~~ **Decided at the plan gate on 2026-09-16: `L3` for four, `L2` for the fifth.** AC-14 makes the harness create, restrict and destroy its seat through `POST /Users/New`, `POST /Users/{userId}/Policy` and `DELETE /Users/{userId}`, and it already lists accounts — so those four are asked of both servers by a run that has to happen anyway. `POST /Users/Password` is on no comparison's path | §6 | Closed. [plan §8](plan.md#8-testing-strategy) |
...
| OQ-13 | ~~How does a command set one property without composing the other forty-one?~~ **Decided at the plan gate on 2026-09-16: it reads the account's policy back and posts it whole**, which the gate made compulsory rather than tidy — a partial body is refused. And `user add` lists the accounts before it creates one, so it can tell the operator which of the two refusals the same six words meant | §3.7 | Closed. [plan §6.7](plan.md#67-the-client-oq-13-decided-here) |
