...
*(Criterion 21 was added at the 2026-09-04 audit — M9. Two rows of §3.2's creation table had
tests and no criterion, and the count they belong to was wrong in §6 as well.)*

21. `POST /Playlists` refuses a body in **four** shapes from three layers, and which one arrives
    is decided by *what* about the body was wrong — §3.2's table, and the four-row extension T8's
    measurement put in [behaviours
    §1.11](../../docs/compatibility/behaviours.md#111-there-are-four-error-shapes-not-one), which
    is where each key and each sentence is written down. Two of the four are AC-2's — a required
    property absent, keyed `$`, and the same property present and null, keyed by its own name. The
    other two are the converter's and the binder's: a `MediaType` in no vocabulary is keyed `$`
    again, with the byte position counted **inside the quoted token** rather than from the start of
    the request, so a one-character value and an eight-character one give different positions on
    the same body; and a value in `Ids` or `UserId` that is not an identifier is keyed on the
    **empty string**, reading `The supplied value is invalid.` Beside them stands AC-3's bare-text
    `400`, which is the same status in a different content type and different bytes. And **no**
    refusal of this body names the action parameter beside its key: that second key belongs to a
    **required** body, and this route — whose four properties may be sent as query parameters
    instead — is the one that separated the two. That last clause is what makes this criterion a
    claim about every refusal of the route rather than about the three somebody listed.

## 6. Conformance

| Endpoint | Level | How it is proven |
|---|---|---|
| `POST /Playlists` | **L2** | Golden response; creation then query; the **four** refusal bodies from three layers, told apart by their keys, and the empty name (AC-2, AC-3, AC-21) |
