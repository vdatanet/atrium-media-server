...
| `pointer` | JSON Pointer to the field or array, relative to the body; `header:<name>` for a header |
...
**`endpoint` and `pointer` are what stop this file from being a list of names**, and they are the
column a later reader will try to normalise away — a single global `ChildCount` row looks like the
same information, and it is the version that excuses the aggregate this server computes.
...
is what a run consults to decide whether a row is even askable before it counts it as a miss.
...
1. **Lengths first.** Different lengths are one `LENGTH` finding, and the array's rows are **not**
   compared at all. This is the cascade guard, and without it AC-2's very first run — where one
   server resolves a multi-part film as two sources and the other as one — buries every real
   finding under positional noise.
...
3. **An array the allowlist marks `unordered` skips step 2 entirely** and compares as a multiset
   ...
   non-total ordering (behaviours §3.6) not being reported as Atrium's defect.
...
| A positional comparison drowns the report in noise on the first real run | High without the guard | High — an unread report is worse than none (spec §6) | The `LENGTH` cascade guard and the `ORDER` class (§6.2), and both are mutation-tested rather than argued |
