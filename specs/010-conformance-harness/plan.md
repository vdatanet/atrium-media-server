...
**And the anchor of §6.1.1 is four kinds rather than one** — three of them the finding that cost
the most, and the fourth what those three could not pair. A `listing:` anchor is the plan's own — a
declared listing case and a row position. It filled 32 of this file's 55 anchors when T6 wrote
it, and fills 31 of today's 66 - the twelve that moved to the fourth kind on 2026-09-07 are
the register's measured mis-pairing. The other kinds
are what the remaining 23 needed, and what a position turned out not to be able to say:
...
**A fourth kind, `named:`, added 2026-09-07, and it is the door §4.4's anchor section could only
describe.** That section says which row each side picked and cannot make them the same row; this
kind makes them the same row. It names a listing case and a **`Name`** rather than a position, and
each side takes the row carrying that name wherever its own ordering puts it. The value is one the
two servers were **measured** to share — over this fixture there are exactly two such tracks,
`Ninety Six Kilohertz` and `Soundless`, 22 such films, three such series and five such albums,
computed from `docs/compatibility/reference-fixture-reading.json` against the twenty renamings of
`tests/library/test_reference_reading.py` — so the pairing is checkable rather than assumed. Its
two consequences are both deliberate: a name neither side has resolves on **neither**, so the case
is reported unasked with the reason instead of comparing two rows that share a position; and a
case that names a fixture item declares `needs: [fixture]` like every other one here, which is
what took the twelve audio cases from `needs: []` and the register's fixture-needing count from
eleven to twenty-three. `check_anchor_orderings` stays keyed on `listing:` and does not widen to
it, because a name is the one anchor shape that survives a listing with no ordering at all.
...
`p` resolves to, through the same kinds and the same per-server resolution, so nothing new is
...
**And from 2026-09-07 the table carries both anchor kinds, which is what makes it worth reading
twice.** A `named:` row reads `paired by name` and shows the **position** each side found the row
at — the names are equal by construction there, so the positions are the only moving part, and two
sides that found one track at two positions are two orderings still disagreeing under an anchor
that no longer cares. The summary line counts what a reader must act on and a pairing is not one
of them.

**A second line joined the provenance the same day, and it is the cause the anchor table was
showing the symptom of.** It says **which libraries each seat can open on each side**, asked of
`/UserViews` per seat. The restricted seat is created by the run on the reference and handed in on
Atrium, so one narrowing is made by two hands against two servers — and on the composed fixture
they parted: `Films` there and `Movies` here, because the reference sorts `/UserViews` by name and
Atrium answers it in declaration order while the choice was *"the first `movies` view with
something in it"*. It is why `movies-by-sort-name@0` read `2 Fast 2 Furious (of 16)` against
`Both Subtitle Kinds (of 15)` for that seat, and why **22 of its 23 unasked cases** said the anchor
listing held zero rows on Atrium. `movies_library` now walks that list **by name**, so both sides
choose alike; the provenance line is what says so on a run where they do not.
