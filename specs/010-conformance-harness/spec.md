...
**Two more were added on 2026-09-04 by [012 T10](../012-negotiation-inputs/tasks.md), and they are
the first rows a feature wrote about *itself*** — declared by its own plan rather than inherited
from a finished feature's list, because 012's two `level: L3` rows need them and 012 was not
finished when they were written.
**The twenty-two rows below are the ones a sweep will not raise**, each for one of five
...
| The hop at which an **un-inspectable source's advertised address** fails (behaviours §3.13) | The sweep compares the negotiation and stops there, and the negotiation is where the two servers *agree* — both answer `200` with the flags decided and a `TranscodingUrl` naming `master.m3u8`. What differs is two requests further on, and no declared case follows an address it was handed | Negotiate the un-inspectable film on both, follow the address, follow the variant line inside it, and compare **which hop** answers the `500`. **Needs the fixture of §3.1**: a source nothing has ever opened exists in no library but this repository's own |
| A negotiation **writing down what it opened** (behaviours §2.23) | Its subject is two requests and their order — a listing, a negotiation, the same listing again — and the engine compares one response at a time, so a listing that *changed* between two identical requests is a difference no single comparison can hold | List the latent film on both, make its bytes readable, negotiate, list again, and compare what each second listing gained. **Needs the fixture of §3.1**: the file has to become readable underneath both servers, which means changing the tree they are mounted over |
...
inheritance — except the two 012 added, which are the first written by the feature they are about.
A harness that reports a clean run without them has proved that the questions it asked have the
same answers, which is a smaller claim than it sounds.

**Four of the rows were added on 2026-09-02, by the decision the task list reserved as D-6**
...
2. **The reference's reading of the built fixture is recorded, and Atrium's scan of the same tree
   is compared against it in the default job**; every difference is declared with its reason and
   its owning feature, an undeclared difference fails, and a declared difference that has gone away
   fails too. *(**Amended 2026-09-02 — D-7, §7.** As accepted on 2026-08-26 this criterion read
   *"both servers, pointed at the same built fixture, produce libraries with the same item count
   and the same structure"*, written before anything had been measured. Measured on 2026-09-02, the
   two readings differ in **forty-seven declared places** over the six libraries the fixture
   composes, and every one of them belongs to 003 or 004 — which §2 puts outside this feature,
   because deciding what Atrium does about a difference this feature finds is the owning feature's.
   The criterion now states the comparison rather than the conformance. It was **unblocked
   2026-09-01** by the single-use reference instance of §3.1, which is what made the reading
   possible at all; before that it was recorded as blocked, on the reading that the only reachable
   reference was an operator's own server.)*
...
16. **Every one of §3.10's twenty-two named comparisons is either run or reported outstanding**, by
    name, in the same report. An outstanding one blocks the run from being called clean (§3.4).
    *(The count moved from sixteen to twenty on 2026-09-02 — D-6, §7 — and to twenty-two on
    2026-09-04, when 012 T10 added the two rows its own L3 conformance needs.)*
...
### Raised by the closing task, and decided

The closing task measured AC-2 and found the criterion contradicted by its own measurement.
Amending an accepted criterion is the shape of D-3 and D-6, so the task reserved the call with a
recommendation rather than taking it.

| # | The call | The decision, taken 2026-09-02 |
|---|---|---|
| D-7 | Whether **AC-2** is amended to state the comparison it turned out to be, or 010 stays open until the forty-seven differences its own measurement declares are closed by the features that own them | **Amend it, and amend nothing else.** AC-2 above now reads as the thing that exists and runs: the reference's reading of the built fixture is recorded, Atrium's scan of the same tree is compared against it in the default job, every difference is declared with its reason and its owning feature, an undeclared difference fails, and a declared difference that has gone away fails too. What it no longer asserts is *"the same item count and the same structure"*, which the measurement contradicts: the two readings differ in **forty-seven declared places** over the six libraries the fixture composes — a zero-byte film that is an item there and not here (003 §3.2), twenty-five files named differently, an empty library that is nothing at all to the reference, and every library's own root row `[probe: tools/probe_reference_scan.py, Jellyfin 10.11.11, 2026-09-02]`. **This is not a harness defect and closing it is not 010's to do**: §2 puts *"deciding what Atrium does about a difference this feature finds"* with the feature that owns the endpoint, and every one of the forty-seven is 003's or 004's. AC-2 was the only one of the eighteen asserting a property of *Atrium's conformance* rather than of the harness, and it was written on 2026-08-26 before anything had been measured. The alternative — holding 010 open — would make this feature's status depend on work in two other features that this feature exists to **report** rather than to do, and leave the harness unusable meanwhile. With it taken, **010 is `Implemented`**: fifteen of fifteen tasks done and eighteen of eighteen criteria proven by tests that assert what the criterion says |
