...
- **Alias validation.** Walk every `AtriumModel` subclass; for each, look up the corresponding
  schema in the pinned OpenAPI document and assert that **every serialisation alias is a property
  name the reference actually uses**. This catches the 5.3% of names a generator gets wrong,
  catches a typo, and catches a field invented by accident. It fails with the model, the field, the
  alias produced and the name expected.
