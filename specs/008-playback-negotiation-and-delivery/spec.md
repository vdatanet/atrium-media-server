...
measured rule at 10.11.11, **for a negotiation the client sent a profile to**: a **single** denied
permission changes nothing. For a video item, `SupportsTranscoding` goes `false` only when
`EnableVideoPlaybackTranscoding`, `EnableAudioPlaybackTranscoding` **and** `EnablePlaybackRemuxing`
are all denied at once; for an audio item, `EnableAudioPlaybackTranscoding` alone decides `[probe:
...
flags, never an `ErrorCode`.

**A negotiation with no profile is gated by a different rule, and it is the opposite one.** Rule 1
below answers direct play because there is nothing to evaluate — and there being nothing to
evaluate is exactly why the gate above never runs: the reference applies it to a *ladder answer*,
and with no profile there is no ladder answer. What the source carries instead is the account's own
permissions, read **one per media kind**: a video item's `SupportsTranscoding` is
`EnableVideoPlaybackTranscoding` and its `SupportsDirectStream` is `EnablePlaybackRemuxing`; an
audio item's `SupportsTranscoding` is `EnableAudioPlaybackTranscoding` and nothing touches its
`SupportsDirectStream`; `SupportsDirectPlay` is untouched by all three `[probe:
tools/probe_playback_info.py, Jellyfin 10.11.11, 2026-09-02; source:
Emby.Server.Implementations/Library/MediaSourceManager.cs:355-372,
Jellyfin.Api/Controllers/MediaInfoController.cs:189 @ v10.11.11]`. So a **single** denial is
observable on `GET /Items/{itemId}/PlaybackInfo` and on a `POST` carrying an empty body, and
invisible on a `POST` carrying a profile — the same account, the same item, two answers. Atrium
reproduces both. At **delivery** the enforcement is stranger still, and it is
...
- **A profile that says nothing means "anything" — and a profile that says *nothing at all* is a
  ...
  told us, and the answer is direct play, with the two capability flags the policy paragraph
  above decides and `SupportsDirectPlay` true — every flag true for the permitted account, which
  is what this rule read until the permissions were measured on this branch. Reading absence as
...
refusals are the `stream` pair's rather than `/universal`'s — every row asked of **both** routes
in one run `[probe: tools/probe_hls.py, Jellyfin 10.11.11, 2026-09-05]`:

| Refusal | `master.m3u8` | `main.m3u8` |
|---|---|---|
| No credential | `401`, empty body | `401`, empty body |
| An item nothing holds | `404`, `text/plain`, the fixed 25 bytes | the same |
| A `mediaSourceId` naming no source | `400`, `text/plain`, the same 25 bytes | the same |
| No query string at all | **the framework's own**: `mediaSourceId` is required on this route, so this is problem details naming that one field | **not a refusal**: a copy is planned at the copy default and a playlist is answered |

**Three of those four rows had only ever been sent to one route, and the table said two.** The
2026-08-29 run asked the tokenless request and the unknown `mediaSourceId` of `master.m3u8` alone
and the empty query of `main.m3u8` alone; nothing had ever sent `main.m3u8` without a credential.
Asked of both routes on 2026-09-05, the first three rows reproduce on each — which turns
`main.m3u8`'s empty `401` from a reading of the controller's authorization attribute into a
measurement ([behaviours §2.10](../../docs/compatibility/behaviours.md)) — and **the fourth does
not generalise**: `master.m3u8` declares `mediaSourceId` required where `main.m3u8` declares it
optional `[source: Jellyfin.Api/Controllers/DynamicHlsController.cs:422, 768 @ v10.11.11]`, so a
bare `master.m3u8` is problem details naming that one field where a bare `main.m3u8` is a
playlist.

> **What Atrium answers to a bare `master.m3u8` is undecided, and it is the one thing this
> correction did not take.** Here that request answers a playlist, where the reference answers
> problem details naming `mediaSourceId` — a difference measured on 2026-09-05 by the run that
> found the over-extension, in a feature that is already `Implemented`. It is a **delta a client
> can observe** (Principle I) and the remedy is to require that field on this route as the
> reference does; but requiring a field a shipped route has been accepting changes what an
> existing caller is answered, so it is recorded here with its measurement and handed to the
> feature's owner rather than improvised by the correction that found it. Until it is decided,
> this row states the reference and not parity, and no criterion in §5 claims it.
...
| No query string at all | **the framework's own**: `runtimeTicks` and `actualSegmentLengthTicks` are required, so this is problem details naming both — and the opposite of what the same treatment of `main.m3u8` answers. It read *"the one refusal on these three routes that is not the third shape"* until `master.m3u8` was asked the same question on 2026-09-05: two of the three refuse an empty query in problem details, each naming its own required parameter |
...
30. A `PlaySessionId` from `PlaybackInfo` is accepted by the delivery route and by
    `ActiveEncodings`.
31. Policy shapes the negotiation exactly as measured (§3.3), and **which rule applies depends on
    whether the request carried a profile**. Against a profile: a user denied **every**
    playback-processing permission negotiates `SupportsTranscoding: false` and no
    `TranscodingUrl`; a user denied only one of them negotiates exactly as a permitted user. With
    **no** profile — the `GET`, and a `POST` whose body and device carry none — one permission per
    media kind decides, so a single denial *is* observable: a video item's `SupportsTranscoding`
    follows `EnableVideoPlaybackTranscoding` and its `SupportsDirectStream` follows
    `EnablePlaybackRemuxing`, an audio item's `SupportsTranscoding` follows
    `EnableAudioPlaybackTranscoding`, and `SupportsDirectPlay` follows none of them. No policy
    shape on either branch produces an `ErrorCode` or a `4xx`. At delivery, a denied user's re-encode
...
32. The four `stream` routes **accept every token mechanism and require none**: a request carrying
    ...
    identically, byte for byte. `/Audio/{itemId}/universal` and the **three HLS routes** — the two
    playlists and the segment — are the four delivery routes that refuse without a token, and all
    four refuse with the empty `401` (§3.5, §3.6, §3.7, behaviours §2.10). The criterion was
    written the other way round until it was measured, named only `/universal` until the playlists
    were measured too, and **said "three" until the 2026-09-04 audit counted the segment route**:
    T11 added that route's own refusal table to §3.7 and extended nothing here, so this criterion
    excluded by its text the route its own acceptance-map row already named (M7).

*(Criteria 33 and 34 were added at the 2026-09-04 audit — M7 and M8. Both are §3 refusal tables
with a full battery of tests and no criterion, so the acceptance map could name none of those
tests and every one of them could have been weakened without a criterion failing. The rows
themselves are unchanged; what was missing was a §5 that claims them.)*

33. The segment route answers the **six** rows measured for it, and only five of them are
    refusals (§3.7). No credential is the empty `401` — the same shape AC-32 counts it under.
    Three are the delivery family's third shape, `text/plain` and the fixed 25 bytes: an item
    nothing holds at `404`, a `mediaSourceId` naming no source at `400`, and a start position
    beside the segment's own at `400` — the last of them with **nothing produced**, because a
    segment states where it begins and two positions in one request have no meaning. The fifth is
    the exception that makes the other four worth stating: a request with **no query string at
    all** is the framework's own refusal, `runtimeTicks` and `actualSegmentLengthTicks` being
    required, so it is problem details naming both where the same treatment of `main.m3u8` answers
    a playlist rather than refusing. And the sixth row is **not** a refusal: a `playlistId` nothing
    named decides nothing and the segment is served, which is what keeps an implementation from
    validating a parameter the reference ignores.
34. `PlaybackInfo`'s own two refusals are two shapes and not one, on **both** the `POST` and the
    `GET` (§3.3): an unknown or invisible item is `404` in problem details, agreeing with `GET
    /Items/{itemId}`'s refusal for that same identifier; a request carrying no token is `401` with
    **no body and no `Content-Type`**, because it is refused before the route runs. The other two
