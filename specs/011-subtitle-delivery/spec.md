...
> ⚠️ **Three cells of the playlist column are wrong or empty, all three are measured, and none of
> them is changed above.** Amending an accepted specification is a decision, and
> [plan §6.8](plan.md#68-what-no-probe-here-has-measured-and-what-stays-owed) made two of these
> the user's; the third was found beside them. They are recorded here with the wording each row
> would take, so that the table's reader sees the measurement even while the row still says
> something else. **The route and its tests answer what was measured, not what the rows say** —
> which is exactly the gap this note exists to make visible.
>
> | Row | What it says | What was measured | Recommended |
> |---|---|---|---|
> | Item id is not an identifier at all | both routes name `routeItemId` | the playlist names **`itemId`** and the fetch route names `routeItemId`: each names its own path segment, and the two spell it differently | playlist cell becomes *`400`, problem details naming `itemId`* |
> | Item id names an item **with nothing servable** | `—` on the playlist | **`404`, problem details** — the reference's lookup asks for a *video*, so a series and an audio track are the same refusal as an identifier nothing holds, where the fetch route answers `500` | playlist cell becomes *`404`, problem details* |
> | *(absent)* the media source states **no runtime** | no row at all | **not measured**: the reference raises its own argument check, which is the `400` a zero window length gets, and no video source in the reference library states no runtime, so the row is a reading `[source: Jellyfin.Api/Controllers/SubtitleController.cs:356-363 @ v10.11.11]` and the run says so | a new row *“Media source states no runtime → `400`, `text/plain` \| —”*, marked as read rather than measured |
>
> All three `[probe: tools/probe_subtitle_delivery.py, Jellyfin 10.11.11, 2026-08-30]`, the first
> two as answers and the third as a reported miss.
