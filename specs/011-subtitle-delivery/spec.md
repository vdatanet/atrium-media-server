...
**That reason is `subrip`'s alone, and the alias beside it parts from its own canonical spelling
somewhere else** *(added 2026-09-05 by the 2026-09-04 audit's M10 pass, which found this
sentence generalising one spelling's reason to both)*: `webvtt` is written by the same writer as
`vtt`, so what separates the two is the label and the switch — `AddVttTimeMap` is read against
`vtt` and not against the alias, as the paragraph above records, so the same request under the two
spellings answers a mapped document and a plain one. Neither spelling is reachable from a playlist
a client follows; both are one hand-made address away.
...
*(Criterion 17 was added at the 2026-09-04 audit — M10. §3.5's format-spelling table had a full
battery of tests and no criterion: AC-13 is written against §3.7's rows, and the table's own two
`200`s under a label nothing chose were named nowhere in §5.)*

17. **Every spelling the writers admit answers `200`, under the media type measured for it, with a
    `Content-Length` equal to its body** (§3.5): `vtt` as `text/vtt`, `srt` as
    `application/x-subrip`, `ass` and `ssa` as `text/x-ssa`, `json` and `js` as
    `application/json`, `ttml` as `application/ttml+xml`, and `subrip` and `webvtt` — the two the
    reference can write and has **no media type for** — under the framework's fallback
    `application/octet-stream` rather than the refusal a missing label suggests. A format with no
    media type is still a body, and a *different* body from its canonical spelling's: on a track
    already in SubRip, `subrip` is rendered, byte order mark and all, where `srt` is AC-10's
    same-format short circuit handing back the artefact's own bytes without one; `webvtt` shares
    `vtt`'s writer and parts from it on the time-map
    switch, which is read against `vtt` and not against the alias. The mark is on every
    rendered document but the cue-list one, and the two cue-list spellings answer one object of
    `TrackEvents` in ticks. A spelling **no** writer admits is §3.7's controller `400` before any
    file is opened — AC-13's row, and the boundary that makes the list above the whole of what
    answers.
...
| `GET …/Subtitles/{index}/Stream.{format}` and its ticks-in-path form | **L2** | Cue-level assertions against a fixture of known cues, whole and windowed, both timestamp switches, both spellings of the path, the format battery over every writable spelling and its label (AC-17), plus determinism (AC-14) |
