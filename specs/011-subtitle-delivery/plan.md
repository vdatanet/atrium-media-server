...
* **The playlist route's malformed-identifier row names the wrong parameter, and it is measured
  ...
  name. **The correction to the accepted spec's table was put to the user rather than taken**:
  the recommended wording is written beside the row in [spec §3.7](spec.md#37-error-paths) and the
  row itself is unchanged, so the table is knowingly one cell behind the measurement until that
  decision is made. Nothing else waits on it.
* **A source with no runtime has no row in §3.7, and the library cannot reach it.** The reference
  ...
  the window length it shares a shape with. **Its row in §3.7 is recommended and not taken**, for
  the same reason as the bullet above.
