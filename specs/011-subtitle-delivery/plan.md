...
anything is `SubtitleMethod=Hls` in the query string, on its own — `SubtitleStreamIndex` beside
it decides which announcement is the default and nothing else, measured at T11 (§6.5). So
...
The invariant assembly of §6.4, plus the two pieces it is built from. Takes the culture index
rather than importing the table, so the table is an argument in a table test — and the index is
the one `library/naming/external.py` already builds, passed in by the caller so that `media/`
gains no dependency on `library/`. *(This block declared one function and a `CultureIndex` type
that does not exist; T10 wrote the three signatures above, over the `dict[str, Culture]` T3
built.)* `first_to_upper` is not `str.capitalize`: it raises a first character only when it is a
lower-case one, and never expands one character into two.
...
**`MediaProbeRepository`** gains
`put_external(library_id, relative_path, files: Sequence[DiscoveredSubtitles])` and returns
...
> **Corrected on 2026-09-05 by the 2026-09-04 audit's M12.** This block named the third argument
> `streams`, and it is **files** — the one thing `DiscoveredSubtitles` exists to say it is not.
> Its own docstring opens *"A file rather than a stream, because the change signal belongs to the
> file"* (`src/atrium/domain/media.py:249`): an `.mks` holds several tracks behind **one**
> `(size, mtime_ns)`, so a per-stream argument would carry that pair once per track with nothing
> keeping the copies equal, and `put_external`'s own docstring calls the difference *"a widening of
> plan section 5's `put_external(..., streams)`"*. **T3 recorded it in its Done note on the day it
> landed and did not fold it back here** (`tasks.md:365-372`) — Principle III's *"it goes back into
> the spec, in the same change"* is what this note is paying, one audit late. The ordinals still
> run across the whole flattened sequence, because an ordinal numbers a *stream*: that half of the
> block was right. No code moves.
...
address, measured — with `-1` the one exception, restated like any other and never written into
the address, which is the third condition of the bullet below.
...
**And a third this section did not have at all, measured at T9: the ladder runs twice, and the
first run is a direct-play condition.** Before any of the above decides what a client is *told*,
the reference resolves the **selected** track's method at `PlayMethod.DirectPlay`, against the
source's own stored container and with no sub-protocol, and adds `SubtitleCodecNotSupported` to
every direct-play entry's failures when that answer is not `External`, `Embed` or `Drop`
`[source: MediaBrowser.Model/Dlna/StreamBuilder.cs:1297-1309 @ v10.11.11]`,
`[probe: tools/probe_subtitle_negotiation.py, Jellyfin 10.11.11, 2026-08-30]`. So `decide()` reads
the subtitle index and the subtitle profiles rather than having subtitle answers applied to its
result afterwards, `_direct_play_reasons` returns the matched `DirectPlayProfile` beside its
reasons, and `Decision` carries `subtitles` and `subtitle_index` on every rung including the
refusal — which the reference annotates too, because its builder describes a stream whatever the
play method was. Nothing is circular: the refusal reads the method *at direct play*, where an
`Hls` entry is skipped outright, and the answer the client is finally given reads it again against
the rung that was reached. Spec §3.3 and AC-15 carry it.

**The output container is not one value.** The `Embed` passes are its only readers, and it is the
transcoding target's container on a produced answer and the **source's own, narrowed to a single
name against the client's direct-play entries**, on a direct play — the reference's
`NormalizeMediaSourceFormatIntoSingleContainer`, which is *not* `media/info.py`'s
`source_container`: that one is a listing's answer and lets the file's extension win, this one
reads the profile. It is `_single_container` in `media/decision.py`, narrowed against the matched
entry alone, which is what the reference passes when it builds the answer.
...
`?ApiKey=` appended `[source: MediaBrowser.Model/Dlna/StreamInfo.cs:1252-1272 @ v10.11.11]`.
**The start position is not `0` for every request this feature can produce**, which is what this
paragraph used to say. It is `0` on every answer whose sub-protocol is HLS, which forces it so
because a playlist preserves timestamps; on a **progressive** transcode it is the negotiation's
own `StartTimeTicks`, and the reference's own ordering comment says as much — it writes the
subtitle addresses *after* the start position is set, on purpose. Measured: a body carrying
`StartTimeTicks=6000000000` against an `http` transcoding target is answered
`…/Subtitles/9/6000000000/Stream.vtt` `[source: MediaBrowser.Model/Dlna/StreamInfo.cs:1179-1181 @
v10.11.11]`, `[probe: tools/probe_subtitle_negotiation.py, Jellyfin 10.11.11, 2026-08-30]`. The
third input is the transcoding profile's `CopyTimestamps`, which this server binds nowhere and
which is `false` for every profile it can read. `IsExternalUrl` is `false` on every one of these
addresses, because the alternative branch serves a remote file and v1 has no remote sources —
measured on the wire beside the address itself, on 1 of the 6 subtitle streams of a negotiated
source and on none of a bare read's. Same probe.
...
* `SubtitleStreamIndex` immediately after `AudioStreamIndex`, written when an index was selected,
  the index is not `-1`, and **either** the method is not `External` **or** the request asked for
  burn-in — the disjunct this bullet was drafted without, and it is observable: a body carrying
  `AlwaysBurnInSubtitleWhenTranscoding: true` beside an external method is answered an address that
  names the index after all, and that same body appends
  `&alwaysBurnInSubtitleWhenTranscoding=true` to the very end of the address, after
  `TranscodeReasons`, in a lower camel case nothing else in it uses — because the reference appends
  it outside `ToUrl`, once the address is already built `[source:
  MediaBrowser.Model/Dlna/StreamInfo.cs:960-963, Jellyfin.Api/Helpers/MediaInfoHelper.cs:325-328 @
  v10.11.11]`, `[probe: tools/probe_subtitle_negotiation.py, Jellyfin 10.11.11, 2026-08-30]`.
  `Switches` gains `always_burn_in_subtitle_when_transcoding` for it: the field was bound on the
  request body at 008 and read by nothing, which is exactly the shape the `SubtitleProfiles`
  comment warned about, and T9 is the task that makes it readable;
...
**`SubtitleProfileDto.Method` binds loosely and refuses strictly**, which §6.8 left owed and T9
measured: `hls`, `HLS` and the ordinal `3` all answer what `Hls` answers, an entry with no
`Method` key at all takes `Encode` — the member no pass of the ladder can return, so it is
indistinguishable from declaring nothing — and a word that is no member is the framework's
validation `400` `[probe: tools/probe_subtitle_negotiation.py, Jellyfin 10.11.11, 2026-08-30]`. A
pydantic enum is case-sensitive, so the field carries a before-validator that matches the five
members case-insensitively and by ordinal and hands anything else to the model untouched, which
keeps the refusal the framework's rather than a second one invented here.
...
**The five localised words — four flags and a marker — are not an approximation.** They are the
words the reference itself writes on an English-configured host: `Hearing Impaired`, `Default`,
`Forced`, `External` and — corrected at T10 — **`Undefined`**. Atrium writes exactly those, so
everything in the string but the language name is parity rather than a decision.

> **T10's correction, 2026-08-30: the marker is `Undefined`, and the sentence this paragraph used
> to make was the wrong sentence.** It read *"they are the literals the reference itself writes
> whenever the localised properties are empty — `Hearing Impaired`, `Default`, `Forced`,
> `External`, `Und`"*, which is a true statement about a state **no served stream is ever in**.
> Every subtitle stream carries all five localised properties filled on the way out of the
> repository `[source: Jellyfin.Server.Implementations/Item/MediaStreamRepository.cs:156-167 @
> v10.11.11]`, measured on 910 of 910 subtitle streams of a real library — so the compiled-in
> fallbacks are unreachable and the words actually written are the **translation table's**, in the
> server's interface culture `[probe: tools/probe_stream_display_title.py, Jellyfin 10.11.11,
> 2026-08-30]`. Four of the five agree with the fallback letter for letter; the marker does not.
> The English table's row is `Undefined` `[source:
> Emby.Server.Implementations/Localization/Core/en-US.json:84 @ v10.11.11]`, so a server writing
> `Und` would write a string **no reference of any configuration writes** — a third string, which
> is precisely what this section's language-name paragraph below warns against. The
> invariant form is defined as what an English-configured reference writes (§3.2), and `Undefined`
> is what it writes.

**The assembly itself is measured rather than read.** All 909 subtitle streams of a real library
whose language name could be read off the wire rebuild **exactly** from their own properties under
the order, the separator, the codec's casing and the title's substring suppression above — every
branch of the assembly reached, including a stream with no language, one that is
hearing-impaired, one from a file beside the media, and a title that swallows an attribute. Same
probe.
...
**The lookup is the index `library/naming/external.py` already builds**, token to culture row with
the first row winning — the same table T3 reproduced the reference's own localisation lookup with,
and the same reason it is first-row-wins there: five rows carry `zho` and only the first is the
plain `Chinese`, so a last-wins index would call every Chinese subtitle track
`Chinese (Traditional)`. The index is passed in rather than imported, so `media/` gains no
dependency on `library/` and the matrix is a table test.

**What that costs is precise and worth writing down, and T10 made it exact in two places.** On a
language whose ISO name carries an alternate spelling the announced name differs from an
English-configured reference's by that alternate — `Spanish; Castilian - Forced - SUBRIP` where
the reference writes `Spanish - Forced - SUBRIP` — and from a Spanish-configured one by the whole
string, which is the divergence §3.2 accepted on the argument that `NAME` is a label a person
reads and `LANGUAGE`, `FORCED`, `DEFAULT` and `URI` are byte-identical. **Measured, that is five
of the 29 language tags a real library carries**: five have a display name carrying an alternate
spelling or a qualifier and therefore cannot equal a platform name in any culture, and the other
24 are a single word — a bound rather than a match, because no English-configured reference is
reachable from here `[probe: tools/probe_stream_display_title.py, Jellyfin 10.11.11, 2026-08-30]`.

**And the index answers tags the reference resolves for nothing**, which is the second half of the
cost and is stated here rather than left to be found. The reference matches a tag with no `-`
against the *platform's* terminological three-letter code alone, so a two-letter tag and a
bibliographic three-letter code name no culture at all and are written as the raw tag with its
first letter raised — `En`, `Ger`, `Chi` `[source:
MediaBrowser.Model/Entities/MediaStream.cs:399-415 @ v10.11.11]`. This project's index keys those
tokens too, so it answers `English`, `German` and `Chinese` where the reference answers the tag.
Every one of the 29 tags the measured library carries is a terminological three-letter code, so
**none of the three shapes was reached** and the probe reports the miss on every run: this is read
rather than measured. It is left as it stands rather than narrowed, because narrowing means a
second, deliberately worse lookup beside the one this project has, and the difference lands inside
the `NAME` divergence §3.2 already accepts rather than beside it.
...
rather than refused: `SubtitleMethod=banana` is no method at all, not a `400`. The value is matched
against the five members case-insensitively **and by ordinal**, in `media/decision.method_named`,
which is where the ordinal table T9 added for the request body already lives — one table, two
binders.

**Both halves of that were measured at T11, and the reason for the first is not the one this
paragraph gave.** It cited behaviours §1.12, the general rule that an unrecognised query *value*
is ignored; what actually decides it is narrower and stronger: Jellyfin binds every **nullable
enum** parameter through a model binder of its own that catches the conversion failure and leaves
the value unset `[source: Jellyfin.Api/ModelBinders/NullableEnumModelBinder.cs:26-46,
Jellyfin.Api/ModelBinders/NullableEnumModelBinderProvider.cs:14-25 @ v10.11.11]`, so `banana`, an
ordinal that names no member, an empty value, `3.0`, `--3` and a non-ASCII digit are each a `200`
announcing nothing, where `hls`, `HLS`, `hLs`, `3`, `+3` and a padded ` 3 ` announce what `Hls`
announces `[probe: manual requests via tools/_probe.py, Jellyfin 10.11.11, 2026-08-30]`. **The
index beside it is typed and does refuse**: `SubtitleStreamIndex=banana` is the framework's problem
details naming it, measured in the same run — so "neither carries a validation pattern" is true and
"neither refuses" is not.

**And a comma list is one value whose parts are OR-ed**, measured on both sides of the
discrimination: `Embed,External` is `1 | 2`, which is the manifest method's own ordinal, so it
announces every text track where `External,External` announces none, and one unreadable part fails
the whole value. `method_named` reproduces it, because a first-name-wins reading would answer the
opposite on both — and the unreadable spellings above are why it does not reach for a bare integer
conversion, which raises on two of them where the reference answers `200`.
...
`hls.master_playlist`. **That condition is right and [spec §3.4](spec.md#34-the-manifest)'s
sentence beside it was not**: the spec said the announcement needs the method *beside a stream
index*, and the index turns out to decide only which entry is the default (measured at T11, same
run). This paragraph never asked for the index, so the code that followed it is the measured
behaviour; the spec is amended rather than the plan. `media/hls.py` renders, verbatim `[source:
...
request's, written the way the reference writes it — which is with nothing at all where the
request named none.

**That last state is unreachable on the reference and reachable here, and this paragraph had it
backwards.** It said a master playlist asked for without the parameter *"announces the reference's
own broken address"*; the parameter is declared **required** there, so an absent one is problem
details naming `mediaSourceId` and no manifest is answered at all `[probe: manual requests via
tools/_probe.py, Jellyfin 10.11.11, 2026-08-30]`. What makes the state reachable here is 008's own
shape — every delivery route binds that parameter optionally — so the empty address is a branch
Atrium can produce and the reference cannot, on a request the reference refuses. Reproducing the
reference's `string.Format` of a null is the narrow answer and is what ships; narrowing 008's
binding is a change to seven routes and belongs to whoever measures the refusal on all of them.
AC-8's traversal starts from a negotiated address, which always carries it.
...
* **The lower-case spelling of `SubtitleMethod` is measured, and it needed four classes rather
  than two.** This bullet said one probe row would settle it either way. It took six, and the
  answers are: `hls` and `HLS` bind exactly as `Hls` does, the member's **ordinal** binds to the
  same member, an entry with no `Method` key at all takes `Encode`, and a word that is no member
  is a `400` `[probe: tools/probe_subtitle_negotiation.py, Jellyfin 10.11.11, 2026-08-30]`. That
  is the same four-class shape 012's gate found for an enum-typed *query* parameter, arriving here
  on a request **body** — so the vocabulary is read loosely and refused strictly, and §6.3 says
  how. **What is settled is the vocabulary**, on the side T9 owns: the value a client *declares*.

  **The other side is measured now, and the prior held for three classes of four.**
  `SubtitleMethod=hls`, `HLS`, `hLs` and the ordinal `3` in a delivery address announce exactly
  what `Hls` announces — but a word that is no member **does not refuse**: it is a `200`
  announcing nothing, and so are an ordinal naming no member and an empty value `[probe: manual
  requests via tools/_probe.py, Jellyfin 10.11.11, 2026-08-30]`. So the strong prior was right
  about how the word is *read* and wrong about how it is *refused*, which is exactly why the row
  said a measurement was needed rather than an inference. The reason is a binder rather than a
  rule about query values: §6.5 cites it, and `media/decision.method_named` reproduces it beside
  the ordinal table this bullet's own half already needed. **Nothing here is owed any longer.**

  Two things were measured beside it and are recorded where they belong rather than here: an
  index that will not bind *does* refuse, in the same address, which §6.5 carries; and the
  announcement does not need an index at all, which is a correction to
  [spec §3.4](spec.md#34-the-manifest) and to AC-5 rather than to this list.
...
* **Two sibling suffixes of the burn-in flag are 008's and are still missing**, found at T9 while
  implementing the third. The reference appends `&allowVideoStreamCopy=false` and
  `&allowAudioStreamCopy=false` to a `TranscodingUrl` from the same three lines that append
  `&alwaysBurnInSubtitleWhenTranscoding=true`, whenever the body denied that copy `[source:
  Jellyfin.Api/Helpers/MediaInfoHelper.cs:315-328 @ v10.11.11]`. Atrium honours both switches in
  the decision and repeats neither in the address. T9 wrote the burn-in one because it is the
  condition partner of a parameter T9 adds and because the field it reads had been bound and
  unread since 008; the other two are about audio and video copying and belong to 008.
...
streams**, and no external stream's `file_index` is ever read by a command. Asserting only "the
indices are contiguous" would not catch that — **and this does not catch the whole class either,
which T12 found by measuring rather than by reading**: it is a claim about `renumber` and the
class also contains every reader of what `renumber` produced. The command builders need their own
row, over a source whose two numbers are stated apart, and the produced bytes need one over the
matrix entry that has a file beside it (§9).
...
|---|---|---|---|
| The wire index and the ffmpeg index are confused somewhere | Medium | High — a delivery command maps the wrong track, silently, only on items that have a sidecar | **This one fired, and T12 found it.** The mitigation as written — two named fields, one pure `renumber` at the repository boundary, a unit test that the two differ wherever externals exist — proves a property of `renumber` and says nothing about what a *command* reads, and `media/ffmpeg.py` read the wire number on both of its builders while `media/extract.py` read the demuxer one. What catches it is a test over a source whose two numbers are **stated apart**, and a produced-bytes test over the one matrix entry that has a file beside it: with the wire number, a remux of that film answers `200` carrying no video stream at all |
