...
1. If the requested format equals the format the readable file is in, **the file's bytes are
   ...
   Jellyfin 10.11.11, 2026-08-30]` — and §6.8 records that it contradicts AC-10 read literally.
...
* **The same-format short circuit contradicts AC-10 as written.** A windowed fetch whose requested
  ...
  The behaviour is reproduced and spec §3.5 records it. **The amendment is still the user's to
  take**: T7 left AC-10's wording alone and put the recommended clause beside it in §3.5, because
  the spec is accepted and a measurement is a reason to amend rather than a licence to.
...
* **AC-9's *"timings that match the source's"* is exact only where the container starts at zero,
  ...
  it is not discovered a third time. **T7 wrote the test and left the wording alone**, for the same
  reason it left AC-10's alone: amending an accepted criterion is the user's. The test derives the
  offset off the container rather than asserting a literal, so it is exact on both builds either
  way, and the wording is now T12's or the user's.
