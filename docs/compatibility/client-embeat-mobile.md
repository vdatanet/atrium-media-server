...
| §0 Stream, image and renderer URLs authenticate by `api_key` with no headers at all | The third shape, accepted on every one of them. The image routes and the four `stream` routes require no token at all; `/Audio/{itemId}/universal` — this client's own stream route — is one of the five delivery routes that do require one, and `?api_key=` is how it is given ([behaviours §2.10](behaviours.md#210-the-image-and-delivery-routes-accept-a-token-and-require-none)) | ✅ |
...
| §1 `MediaSources[0].Id`, `.Container`, `.Bitrate`, `RunTimeTicks` | All four emitted ([`media/info.py:410-438`](../../src/atrium/media/info.py)) — three of them only where an inspection exists, which since 012 is a state anything that negotiates the item leaves: the inspection taken inside the request is **written down**, so the next listing carries it ([`library/inspection.py`](../../src/atrium/library/inspection.py)). On a listing alone the empty shape is what a stock reference answers too, measured — [§5.1](#51-a-source-with-no-stored-inspection-loses-the-music-clients-whole-negotiation)'s claim to the contrary was withdrawn at 012's gate | ✅ since 012 |
| §1 `MediaStreams[Audio].Codec`/`SampleRate`/`BitDepth`/`Channels` | All four emitted ([`media/info.py:371-375`](../../src/atrium/media/info.py)) — and this is the load-bearing one, since `SampleRate` decides the cast path. Since 012 the negotiation that finds them missing opens the file and keeps what it learns, so the row above's cure is this row's too | ✅ since 012 |
...
| §4 `RunTimeTicks` must be right, or an unsized stream stalls at the end | 005 emits it; §5.1 was where it went missing, and 012's on-demand inspection is what puts it back — on the **source**. The **item-level** `RunTimeTicks` is a second field and is still `null` on every file-backed item, which is [012 D-2](../../specs/012-negotiation-inputs/plan.md#d-2--the-item-level-runtimeticks-which-is-not-this-features) and 003's | ✅ on the source, 🟠 on the item |
