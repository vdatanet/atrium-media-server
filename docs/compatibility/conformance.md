...
| `DateCreated` | **A container's only, from 2026-09-06**: the moment each server's own scan made the row. A file-backed item's is its file's modification time on both servers and is compared — see [behaviours §2.29](behaviours.md) | `wall-clock` |
