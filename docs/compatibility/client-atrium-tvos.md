...
| §1 `SupportsDirectPlay`/`SupportsDirectStream` reflect reality | Decided against the profile for every source since 012 T5, inspection or no inspection: the negotiation opens the file first, and where it cannot be opened the flags are still the ladder's answer rather than the model's defaults ([`library/inspection.py`](../../src/atrium/library/inspection.py), [`api/media_info.py`](../../src/atrium/api/media_info.py)) | ✅ since 012, and [§4.1](#41-a-source-with-no-stored-inspection-is-the-clients-documented-dead-end) is what it was |
| §1 `TranscodingUrl` present whenever direct is refused | The reference's own condition, transcribed ([`api/media_info.py:381-388`](../../src/atrium/api/media_info.py)) — **including** the case above since 012 T5, which is that feature's AC-4. Whether the address then plays is [behaviours §3.13](behaviours.md), where both servers end at the same `500` one hop apart | ✅ |
...
| §1 `DeviceProfile.TranscodingProfiles[].Protocol` selects HLS | A two-member enumeration since 012 T9, bound in any case and by its declared ordinal like every other vocabulary this body carries ([`media/decision.py`](../../src/atrium/media/decision.py), [`compat/model.py`](../../src/atrium/compat/model.py)) — and the answer echoes the enumeration's spelling, never the profile's. It was compared case-sensitively against `"hls"` where `/universal` normalised, which is what [§4.6](#46-two-spellings-of-hls-and-only-one-of-them-selects-hls) found | ✅ since 012 |
