...
version-bump command. **010 is still `Accepted` and not `Implemented`, and its own closing task is
why.** T15 found the class that task exists for: **AC-11 had no test at all** — the plan mapped it to
...
probe prints, which is the mechanism Principle II rests on; and **AC-2 says what its own measurement
...
AC-2 is **D-7**, reserved for its owner rather than improvised, and until it is taken every document
says `Accepted`. 010 also found two differences in **implemented** features, both left to their
owners: a seat with all three playback permissions denied negotiates `SupportsTranscoding: true`
here and `false` there (008's, behaviours §2.21), and Atrium's `is_hidden` default answers a
